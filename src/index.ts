 ...
 *
 * @example
 * import Aptos from "@razorlabs/ledgerjs-hw-app-aptos";
...
export default class Aptos {
  transport: Transport;
	...
	constructor(transport: Transport, scrambleKey = 'default_aptos_scramble_key', verbose = false) {
  ...
  /**
	...
	async getPublicKey(path: string, displayOnDevice = false): Promise<GetPublicKeyResult> {
		const cla = 0x5B;
		const ins = 0x05;
		const p1 =  displayOnDevice ? 0x01 : 0x00;
		const p2 = 0x00;
  ...
  /**
   * Retrieve the app name on the attached Ledger device.
   */
  async getAppName(): Promise<string> {
    const name = await this.#sendChunks(0x5B, 0x04, 0x00, 0x00, Buffer.alloc(1));
  ...
  /**
	...
	async getVersion(): Promise<GetVersionResult> {
		const [major, minor, patch] = await this.#sendChunks(0x5B, 0x03, 0x00, 0x00, Buffer.alloc(1));
		return {
			major,
			minor,
			patch,
  ...
  /**
	...
	async #sendChunks(
		...
		this.#log(data);
		const startBuffer = Buffer.from([HostToLedger.START]);
		return await this.#handleBlocksProtocol(
			cla,
			ins,
			p1,
			p2,
			Buffer.concat([startBuffer, ...parameterList] as Buffer[]),
			data,
		);
	}
